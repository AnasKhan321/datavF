...
export default function Search() {
    ...
    const fethcData = () => {
        const searchh = location.search.split('?')[1]
        const res = fetch(`http://localhost:8001/data/${searchh}`).then((res) => res.json())
            .then((data) => {
                setdata2(data)
            });
            ...
            {displayedData.map((e, i) => {
                ...
                return (
                    ...
                    <div className="my-12 shadow-lg p-4 mx-2  rounded shadow-sky-200 cursor-pointer hover:shadow-sky-300 " key={i}>
                        ...
                        <h5 className="text-right mr-14 text-3xl font-bold  ">{e.topic.toUpperCase()}</h5>
